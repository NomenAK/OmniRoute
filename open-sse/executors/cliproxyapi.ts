...
export class CliproxyapiExecutor extends BaseExecutor {
  ...
  transformRequest(
    ...
    if (this.isAnthropicShape(transformed)) {
      ...
      // ({effort:"low"|"medium"|"high"}) which Anthropic accepts as part of
      // the Claude Code wire image. Strip raw Capy/SDK shapes (extras like
      // unsupported effort tiers or extra fields) that trigger the extras
      // billing gate. buildClaudeCodeCompatibleRequest produces CC-spec
      // shapes; raw Capy bodies don't.
      const oc = transformed.output_config;
      if (oc && typeof oc === "object") {
        const ocRec = oc as Record<string, unknown>;
        const validEffort =
          ocRec.effort === "low" || ocRec.effort === "medium" || ocRec.effort === "high";
